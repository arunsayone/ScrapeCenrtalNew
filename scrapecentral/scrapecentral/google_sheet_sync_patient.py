...
class GoogleSpreedSheetSync(object):
	...
	def sync_data_from_sheet(self, client):
		...
		for row in rows[1:]:
			...
			if rows[7]:
				workflow_session = Session(engine)
				workflow_session.add(workflow(type=rows[7], istop=rows[8], records=rows[9], labs=rows[10], MMP=rows[11], billing=rows[12], note=rows[13],  visit_id=visit_obj[0]))
					...
					if dmo_obj:
						...
						if appointment_obj:
							...
							if pf_obj:
								...
								# set patient_flag
								session.execute("update patient set patient_flag='1' where id='"+str(patient_ob.id)+"'")
								# session.commit()
								session.close()
