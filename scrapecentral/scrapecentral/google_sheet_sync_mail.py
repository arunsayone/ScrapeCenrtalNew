...
class GoogleSpreedSheetSync(object):
	...
	def __init__(self):
		client = self.login()
		self.sheet = client.open("Mailer updated 2-6- Michael").sheet1
	...
	def sync_data_from_db(self, client):
		...
		for patient_ob in patient_obj:
			...
			row_count = len(rows)
			new_row = row_count + 1
			patient_id = patient_ob.id

			# add Id
			cell_name = 'A'+ str(new_row)
			self.sheet.update_acell(cell_name, patient_id)
						...
						if email_obj:
							...
							if appointment_obj:
								...
								if pf_obj:
									...
									#Change mail_flag
									session.execute("update patient set mail_flag='1' where id='"+str(patient_ob.id)+"'")
									session.commit()
